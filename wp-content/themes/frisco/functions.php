...
/* ------------------------------------------------------------------------ */
	...
	load_theme_textdomain( 'smitten', get_template_directory() . '/framework/languages' );
	load_theme_textdomain( 'smitten-framework', get_template_directory() . '/framework/languages' );
...
/* ------------------------------------------------------------------------------------- */
	...
	include_once('framework/inc/widgets/widget_embed.php');
	include_once('framework/inc/widgets/widget_facebook.php');
	include_once('framework/inc/widgets/widget_flickr.php');
	include_once('framework/inc/widgets/widget_advertising.php');
	include_once('framework/inc/widgets/widget_contact.php');
	include_once('framework/inc/widgets/widget_portfolio.php');
	include_once('framework/inc/widgets/widget_custommenu.php');
	include_once('framework/inc/widgets/widget_testimonials.php');
	...
	include_once('framework/inc/cpt-faq.php'); // FAQ
	...
	add_action('tgmpa_register', 'smitten_register_required_plugins');
	function smitten_register_required_plugins() {
		$plugins = array(
			array(
			...
			array(
				'name'     				=> 'WordPress Live Chat', // The plugin name
		...
		$theme_text_domain = 'smitten-framework';
...
/* ------------------------------------------------------------------------ */
	...
	function smitten_comment( $comment, $args, $depth ) {
	   ...
	   <div id="comment-<?php comment_ID(); ?>" class="comment-body clearfix"> 
	         ...
	         <div class="comment-text">
				 ...
				 <div class="author">
				 	...
				 	<div class="date">
				 	<?php printf(__('%1$s at %2$s', 'smitten'), get_comment_date(),  get_comment_time() ) ?></a><?php edit_comment_link( __( '(Edit)', 'smitten'),'  ','' ) ?>
		         ...
		         <em><?php _e( 'Your comment is awaiting moderation.', 'smitten' ) ?></em>
		         <br />
		      	<?php endif; ?>
   ...
   function pagination($pages = '', $range = 4) {
		...
		if(1 != $pages) {
			echo "<span class='allpages'>" . __('Page', 'smitten') . " ".$paged." " . __('of', 'smitten') . " ".$pages."</span>";
			if($paged > 2 && $paged > $range+1 && $showitems < $pages) echo "<a href='".get_pagenum_link(1)."'>&laquo; " . __('First', 'smitten') . "</a>";
			if($paged > 1 && $showitems < $pages) echo "<a href='".get_pagenum_link($paged - 1)."'>&lsaquo; " . __('Previous', 'smitten') . "</a>";
			...
			if ($paged < $pages && $showitems < $pages) echo "<a href=\"".get_pagenum_link($paged + 1)."\">" . __('Next', 'smitten') . " &rsaquo;</a>";
			if ($paged < $pages-1 &&  $paged+$range-1 < $pages && $showitems < $pages) echo "<a href='".get_pagenum_link($pages)."'>" . __('Last', 'smitten') . " &raquo;</a>";
		}
	}
	...
	if($data['check_readmore'] != "0") { // Admin Option Check
		function new_excerpt_more($more) {
			global $post;
			return '… <a href="'. get_permalink($post->ID) . '" class="read-more-link">' . '' . __('read more', 'smitten') . ' &rarr;' . '</a>';
		}
	...
	if ( function_exists( 'add_image_size' ) ) {
		add_image_size( 'standard', 572, 252, true );			// Standard Blog Image
		add_image_size( 'blog-masonry', 280, 180, true );		// Masonry Blog Image
		add_image_size( 'column2-portfolio', 460, 300, true );	// Portfolio - 2 Column
		add_image_size( 'column3-portfolio', 300, 196, true ); 	// Portfolio - 3 Column
		add_image_size( 'column4-portfolio', 220, 160, true );	// Portfolio -42 Column
		add_image_size( 'thumb-blog', 170, 120, true );			// Blog thumbnail
		add_image_size( 'thumb-portfolio', 208, 147, true );	// Portfolio thumbnail
	...
	$topwidgetscolumns = "four";

	if($data['select_topwidgetscolumns'] == "4"){ $topwidgetscolumns = "four"; }
	elseif($data['select_topwidgetscolumns'] ==  "3"){ $topwidgetscolumns = "one-third"; }
	elseif($data['select_topwidgetscolumns'] ==  "2"){ $topwidgetscolumns = "eight"; }
	elseif($data['select_topwidgetscolumns'] ==  "1"){ $topwidgetscolumns = "sixteen"; }
	...
	if (function_exists('register_sidebar')) {
		...
		register_sidebar(array(
			'name' => __('Blog Widgets','smitten-framework' ),
			'id'   => 'blog-widgets',
			'description'   => __( 'These are widgets for the Blog sidebar.','smitten-framework' ),
		...
		register_sidebar(array(
		   'name' => __('Footer Widgets','smitten-framework' ),
		   'id'   => 'footer-widgets',
			'description'   => __( 'These are widgets for the Footer.','smitten-framework' ),
		...
		register_sidebar(array(
		   'name' => __('Upper Widgets','smitten-framework' ),
		   'id'   => 'topwidgets-widgets',
			'description'   => __( 'These are widgets for teh Upper Widget area.','smitten-framework' ),
			'before_widget' => '<div id="%1$s" class="widget %2$s '.$topwidgetscolumns.' columns">',
			'after_widget'  => '</div>',
			'before_title'  => '<h3>',
			'after_title'   => '</h3>'
	   	));
		...
		if (class_exists('Woocommerce')){
			register_sidebar(array(
				'name' => __('Shop Widgets','smitten-framework' ),
				'id'   => 'shop-widgets',
				'description'   => __( 'These are widgets for the Shop sidebar.','smitten-framework' ),
				'before_widget' => '<div id="%1$s" class="widget %2$s">',
				'after_widget'  => '</div>',
				'before_title'  => '<h3 class="title"><span>',
				'after_title'   => '</span></h3>'
